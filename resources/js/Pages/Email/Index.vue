<template>
  <MainLayout>
    <div class="container-fluid py-2">
          ...
          <div class="card">
            <div class="card-body text-center py-5">
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control mx-auto" style="max-width: 400px;" v-model="hostname"
            ...
            <div class="card">
              <div class="card-header pb-0">
                <ul class="nav nav-tabs" role="tablist">
                  <li class="nav-item">
                    <a class="nav-link" :class="{ active: activeTab === 'accounts' }" href="#"
                      @click.prevent="activeTab = 'accounts'">
                      <i class="material-symbols-rounded text-sm me-1">person</i> Email Accounts
                    </a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" :class="{ active: activeTab === 'domains' }" href="#"
                      @click.prevent="activeTab = 'domains'">
                      <i class="material-symbols-rounded text-sm me-1">dns</i> Domains
                    </a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" :class="{ active: activeTab === 'forwarders' }" href="#"
                      @click.prevent="activeTab = 'forwarders'">
              ...
              <div class="card-body">
                      ...
                      <tbody>
                        <tr v-for="account in accounts" :key="account.id">
                          ...
                          <td>
                            ...
                            <button class="btn btn-link text-warning p-1" title="Change Password"
                              @click="showPasswordModal(account)">
                    ...
                    <table class="table align-items-center mb-0">
                      ...
                      <tbody>
                        <tr v-for="domain in domains" :key="domain.id">
                          ...
                          <td>
                            <button class="btn btn-link text-danger p-1" title="Disable"
                              @click="confirmDisableDomain(domain)">
                ...
                <div v-if="activeTab === 'settings'">
                  ...
                  <p class="text-sm text-muted mb-4">Use these settings to configure your email client (Outlook,
                    Thunderbird, etc.)</p>
      ...
      <!-- Enable Domain Modal -->
      <div class="modal fade" :class="{ show: showEnableDomainModal }"
        :style="showEnableDomainModal ? 'display: block;' : ''" tabindex="-1">
      ...
      <!-- Create Alias Modal -->
      <div class="modal fade" :class="{ show: showCreateAliasModal }"
        :style="showCreateAliasModal ? 'display: block;' : ''" tabindex="-1">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
      ...
      <div class="modal fade" :class="{ show: showChangePasswordModal }"
        :style="showChangePasswordModal ? 'display: block;' : ''" tabindex="-1">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header bg-dark text-white">
              ...
              <span v-else-if="terminalStatus === 'complete'" class="badge bg-success ms-2">Complete</span>
              <span v-else-if="terminalStatus === 'failed'" class="badge bg-danger ms-2">Failed</span>
            </div>
            <div class="modal-body p-0">
              <pre ref="terminalOutput" class="terminal-output">{{ terminalLog }}</pre>
            </div>
            <div class="modal-footer">
...
const terminalStatus = ref('idle') // idle, running, complete, failed
const terminalTitle = ref('Installation Progress')
...
const installMailServer = async () => {
  ...
  terminalTitle.value = 'Installing Mail Server'
...
const startLogPolling = () => {
  ...
  pollInterval = setInterval(async () => {
    try {
      const response = await axios.get('/email/install-log')
...
const closeTerminal = async () => {
  ...
  if (terminalStatus.value === 'complete') {
    await loadStatus()
    if (status.value.installed) {
      await loadData()
    }
